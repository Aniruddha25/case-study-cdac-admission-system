...
#define condition5   preferences[k].getCourseName() == courses[j].get_name()
#define condition6   preferences[k].getPrefNo() == (n+1)
#define condition7   students[i].get_degree_marks()>qmarks
using namespace std;
class student {
private:
	int id;//form number
		...
		for(unsigned i=0;i<students.size();i++)
		{
			for(unsigned k =0;k<preferences.size();k++)
			{
				...
				for(unsigned j =0;j<courses.size();j++)
				{
					...
					{
						if(condition2 && condition5 && condition3 &&  condition6 && condition7)
						{
